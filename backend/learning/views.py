...
class ChapterStatusUpdateView(APIView):
    def patch(self, request, chapter_id):
        ...
        if new_status == "in_progress" and not chapter.started_at:
            chapter.started_at = timezone.now()
        elif new_status == "completed" and not chapter.completed_at:
...
class AssessmentGenerateView(APIView):
    def post(self, request):
        ...
        pass_percentage = int(data.get("pass_percentage", 65))
