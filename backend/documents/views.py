...
def run_document_processing(document_id):
    ...
    try:
        ...
        logger.info("LocalMind: Background processing complete for document %s", document_id)
    except Exception as exc:
        logger.exception("LocalMind: failed to process document %s in background", document_id)
        try:
            document = Document.objects.get(pk=document_id)
...
class ProcessDocumentView(APIView):
    """Parse a document and persist actual Chapter -> Module source text in a background thread."""
    ...
    def post(self, request, document_id):
        ...
        if is_testing:
            ...
            from documents.services.worker import run_in_background
...
class DocumentListView(APIView):
    def get(self, request):
        from learning.models import MicroModule
        documents = Document.objects.all().order_by("-created_at")
        data = []
        for doc in documents:
            total_mods = MicroModule.objects.filter(document=doc).count()
            completed_mods = MicroModule.objects.filter(document=doc, status="completed").count()
            progress = round((completed_mods / total_mods * 100.0), 1) if total_mods > 0 else 0.0
